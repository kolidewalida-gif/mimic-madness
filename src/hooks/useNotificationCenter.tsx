...
const load = (): CenterNotification[] => {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? (parsed as CenterNotification[]) : [];
  } catch {
    return [];
  }
};

const persist = (items: CenterNotification[]) => {
  try {
...
/**
 * useNotificationCenter — a lightweight, Realtime-fed notification centre.
 *
 * Aggregates awareness events from systems that already exist (no extra DB
 * tables or triggers required):
 ...
 *
 * NOTE: kept separate from the DB-backed `useNotifications` hook (likes /
 * reactions / follows on a `notifications` table) so both can coexist.
 */
export const useNotificationCenter = () => {
  ...
  const [items, setItems] = useState<CenterNotification[]>(() => load());
  ...
  const push = useCallback((incoming: Omit<CenterNotification, 'read' | 'ts'> & { ts?: number }) => {
