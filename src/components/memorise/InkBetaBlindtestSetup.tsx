...
const OptionToggle = ({
  ...
  <button
    type="button"
    className="bt3-toggle menu-focus"
    data-on={checked || undefined}
    aria-pressed={checked}
    onClick={onClick}
  >
    <span className="bt3-toggle-icon"><Icon aria-hidden="true" /></span>
    <span className="bt3-toggle-copy"><strong>{label}</strong><small>{description}</small></span>
    <span className="bt3-toggle-switch" aria-hidden="true"><i /></span>
  </button>
);
...
export const InkBetaBlindtestSetup = ({
  ...
  if (!isHost) {
    return (
      <motion.section
        className="bt3-waiting"
        initial={{ opacity: 0, y: 18 }}
        animate={{ opacity: 1, y: 0 }}
        aria-live="polite"
      >
        <div className="bt3-waiting-art" aria-hidden="true">
          <div className="bt3-waiting-record"><Disc3 /></div>
          <div className="bt3-waiting-wave">
            {Array.from({ length: 24 }, (_, index) => <i key={index} />)}
          </div>
        </div>
        <div className="bt3-waiting-copy">
          <span className="bt3-eyebrow">Blindtest · Backstage</span>
          <h2>Le mix<br />arrive.</h2>
          <p>L’hôte prépare la sélection. Branche tes écouteurs et garde un doigt sur les réponses.</p>
          <span className="bt3-connection" data-ready={canStart || undefined}>
  ...
  return (
    <motion.section
      ...
      aria-labelledby="bt3-setup-title"
    >
      <header className="bt3-setup-hero">
        <div className="bt3-setup-title">
          <span className="bt3-eyebrow">Ink Beta · Blindtest</span>
          <h1 id="bt3-setup-title">Fais ta setlist.</h1>
          <p><Headphones aria-hidden="true" /> Dix univers, quatre choix, une seule bonne réponse.</p>
        </div>
        <div className="bt3-setup-numbers" aria-label="Sélection actuelle">
      ...
      <section className="bt3-crate" aria-labelledby="bt3-categories-title">
        <div className="bt3-crate-heading">
          <div>
            <span className="bt3-step">01</span>
            <h2 id="bt3-categories-title">Pioche tes sons</h2>
          </div>
          <p>Fais défiler les pochettes · garde au moins un univers</p>
        </div>

        <div className="bt3-crate-track">
          {CATEGORIES.map((category, index) => {
            ...
            return (
              <motion.button
                key={category}
                type="button"
                className="bt3-sleeve menu-focus"
                data-active={active || undefined}
                aria-pressed={active}
                onClick={() => toggleCategory(category)}
                style={{ '--bt3-cat': meta.color } as CSSProperties}
                whileTap={{ scale: 0.97 }}
              >
                <span className="bt3-sleeve-top">
                  <span>{String(index + 1).padStart(2, '0')}</span>
                  <span className="bt3-sleeve-check">{active ? <Check aria-hidden="true" /> : '+'}</span>
                </span>
                <span className="bt3-sleeve-art" aria-hidden="true">
                  <i /><i /><i />
                  <b>{meta.emoji}</b>
                </span>
                <span className="bt3-sleeve-copy">
      ...
      <section className="bt3-mixer" aria-labelledby="bt3-mixer-title">
        <div className="bt3-mixer-intro">
          <span className="bt3-step">02</span>
          <div>
            <h2 id="bt3-mixer-title">Règle le tempo</h2>
            <p>Environ {estimatedMinutes} min de jeu</p>
          </div>
        </div>

        <div className="bt3-rule">
          <label><Disc3 aria-hidden="true" /><span>Manches</span><strong>{playableRounds}</strong></label>
          <div className="bt3-segments" data-count={BLINDTEST_ROUND_OPTIONS.length}>
        ...
        <div className="bt3-rule">
          ...
          <div className="bt3-segments" data-count={BLINDTEST_LISTEN_OPTIONS.length}>
        ...
        <div className="bt3-bonuses" aria-label="Options de partie">
        ...
        <motion.button
          type="button"
          className="bt3-launch menu-focus"
          onClick={() => onStart(Array.from(selected), config)}
          disabled={!canStart || starting}
          aria-busy={starting}
          whileTap={canStart && !starting ? { scale: 0.98 } : undefined}
        >
          <span className="bt3-launch-icon">{starting ? <Loader2 className="animate-spin" aria-hidden="true" /> : <Play aria-hidden="true" />}</span>
      ...
      <div className="bt3-setup-message" aria-live="polite">
