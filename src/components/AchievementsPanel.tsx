...
export const AchievementsPanel = (props: AchievementsPanelProps) => {
  ...
  return (
    <InkDrawer isOpen={props.isOpen} onClose={props.onClose} title="Badges" subtitle={`${progress.unlocked}/${progress.total} débloqués`} icon={<Trophy className="h-5 w-5" strokeWidth={2.5} />} iconGradient="linear-gradient(135deg, #fbbf24, #f97316)" toolbar={toolbar}>
      {content}
    </InkDrawer>
  );
};
    ...
    <motion.article initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index, 12) * 0.04 }} className={cn('relative rounded-xl p-3.5 transition-all', isUnlocked ? 'bg-white/[0.03]' : 'bg-white/[0.015] opacity-60')} style={{ border: `1px solid ${isUnlocked ? `${style.color}33` : 'rgba(255,255,255,0.05)'}` }}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2"><h3 className={cn('truncate text-sm font-semibold', isUnlocked ? 'text-white' : 'text-white/50')}>{achievement.title}</h3><span className="flex-shrink-0 rounded-md px-1.5 py-0.5 text-[10px] font-medium" style={{ background: isUnlocked ? `${style.color}18` : 'rgba(255,255,255,0.05)', color: isUnlocked ? style.color : 'rgba(255,255,255,0.3)' }}>{style.label}</span></div>
