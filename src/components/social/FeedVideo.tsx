...
interface FeedVideoProps {
  clipId: string;
  /** When true this is the single active preview; inactive tiles stay paused. */
  playing?: boolean;
  /** When true the active video plays with sound at `volume`, otherwise muted */
  soundActive?: boolean;
  /** 0..1 */
  volume?: number;
  className?: string;
 ...
 */
export const FeedVideo = ({ clipId, playing = false, soundActive = false, volume = 0.7, className }: FeedVideoProps) => {
  ...
  // Apply mute / volume and guarantee that only the selected preview plays.
  useEffect(() => {
  ...
  }, [playing, soundActive, volume, url]);

  // Pause when scrolled out of view; never start an inactive grid tile.
  useEffect(() => {
    const el = ref.current;
